...
import { RoadmapStatus } from "@/components/dashboard/RoadmapStatus";
...
export default function OverviewPage() {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-slate-800/80 pb-5">
        <div>
          <div className="flex items-center gap-2.5">
            <h1 className="text-xl font-bold tracking-tight text-white font-sans">
              Fraud-Ring Intelligence Command Center
            </h1>
            <span className="px-2 py-0.5 rounded text-[10px] font-mono font-bold bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 flex items-center gap-1.5">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" aria-hidden="true" />
              LIVE TELEMETRY
            </span>
          </div>
          <p className="text-xs text-slate-400 mt-1 max-w-2xl leading-relaxed">
            Multi-signal payment risk monitoring & graph cluster detection for the Razorpay payment ecosystem
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <button
            ...
            aria-label="Simulate Coordinated Fraud Attack"
            className="flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-bold font-mono bg-gradient-to-r from-red-600 via-rose-600 to-red-700 hover:from-red-500 hover:to-rose-600 text-white transition-all shadow-lg shadow-red-950/50 border border-red-400/40 hover:scale-[1.02] active:scale-[0.98]"
          >
            <Zap className="w-4 h-4 fill-white animate-pulse" aria-hidden="true" />
          ...
          <button
            ...
            aria-label="Reset Attack Simulation State"
            className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-mono bg-slate-900/90 border border-slate-700/80 text-slate-300 hover:text-white hover:bg-slate-800 transition-all"
          >
          ...
          <button
            onClick={loadData}
            disabled={refreshing}
            aria-label="Sync Real-time Telemetry"
            className="flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-mono bg-slate-900/90 border border-slate-700/80 text-slate-300 hover:text-white hover:border-slate-500 transition-all disabled:opacity-50"
          >
            <RefreshCw className={`w-3.5 h-3.5 ${refreshing ? "animate-spin text-emerald-400" : ""}`} aria-hidden="true" />
            <span className="hidden sm:inline">Sync Telemetry</span>
          </button>

          <Link
            href="/investigations"
            className="flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-semibold bg-emerald-600 hover:bg-emerald-500 text-white transition-all shadow-sm"
          >
            <SearchCheck className="w-3.5 h-3.5" aria-hidden="true" />
      ...
      {/* Enterprise Security Architecture Module Health Grid */}
      <div className="pt-2">
        <RoadmapStatus />
      </div>
    </div>
